...
def multiply_cols(args):
    """Cada worker multiplica A por su bloque de columnas de B."""
    A, B_chunk = args
    return A @ B_chunk

def parallel_matmul_cols(A, B, num_workers):
    col_chunks = np.array_split(B, num_workers, axis=1)
    with Pool(processes=num_workers) as pool:
        results = pool.map(multiply_cols, [(A, chunk) for chunk in col_chunks])
...
if __name__ == "__main__":
    ...
    C_serial = A @ B
    C_parallel = parallel_matmul_cols(A, B, num_workers=2)
